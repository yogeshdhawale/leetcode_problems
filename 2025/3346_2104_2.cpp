...
public:
    ...
    int maxFrequency(vector<int>& nums, int k, int numOperations) {
        ...
        map<int, vector<int>> fr2Elements;
        for (auto& it : freqMap) {
            fr2Elements[it.second].push_back(it.first);
            if (it.second > numOperations) {
                LOG("At start, returning " << it.second);
        ...
        for (auto& it : fr2Elements) {
            auto curCount = it.first;
            for (auto& curNum : it.second) {
                auto itr = freqMap.find(curNum);
            }
        }
    }
