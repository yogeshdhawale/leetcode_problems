...
public:
    ...
    vector<vector<int>> cell_step;
    vector<int> portal_used_step;
    vector<vector<IDX2D>> portal_nbrs;
    ...
    void init(INPUT_TYPE& matrix)
    {
        ...
        cell_step.resize(rows, vector<int>(cols, -1));
    ...
    int minMoves(vector<string>& matrix) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                portal_nbrs[portalid].push_back({ i, j });
            }
        }
        ...
        int curStep = 0;
        vector<vector<int>> curTrace;
        curTrace.resize(rows);

        // process first cell
        if (isPortalChar(firstCell)) {
            AddPortalNeighbors(firstCell - 'A', curTrace, 0);
        }
        else {
            cell_step[0][0] = 0;
            curTrace[0].push_back(0);
        }

        vector<vector<int>> nextTrace;
        nextTrace.resize(rows);
        ...
        for (; curStep < rows * cols + 1; curStep++) {
            ...
            for (int i = 0; i < rows; i++) {
                for (auto j : curTrace[i]) {

                    if (i == rows - 1 && j == cols - 1) {
                        LOG("Found path at step:" << curStep);
                        return curStep;
                    }
                    AddCellNeighbors(i, j, matrix, nextTrace, curStep + 1);
                }
            }
    ...
    void AddPortalNeighbors(int portalIdx, vector<vector<int>>& nextTrace, int nextStep) {

        if (portal_used_step[portalIdx] != -1) return;
        portal_used_step[portalIdx] = nextStep;

        int cnt = 0;
        for (auto [i, j] : portal_nbrs[portalIdx]) {
            if (cell_step[i][j] != -1) continue;
            nextTrace[i].push_back(j);
            cell_step[i][j] = nextStep;
            cnt++;
        }
        LOG("\tAddPortalNeighbors for:" << char(portalIdx + 'A') << " step:" << nextStep << " added cells cnt:" << cnt);
    }
    void AddCellNeighbors(int r, int c, const INPUT_TYPE& matrix, vector<vector<int>>& nextTrace, int nextStep) {
        for (auto& delta : neighborDeltas) {
            int i = r + delta.first;
