...
struct node {
    ...
    vector<map<int, set<int>>> dp; // k(level) -> (weight, set(neighbors))

    node(int id, int k) : nid(id) {
        dp = vector<map<int, set<int>>>(k + 1, map<int, set<int>>());
    }
    void set_nid(int id) { nid = id; }

    void add(int lvl, int wt, int nbr) {
        dp[lvl][wt].insert(nbr);
    }
    void add(int lvl, int wt, set<int>& nbrs) {
        dp[lvl][wt].insert(nbrs.begin(), nbrs.end());
    }
};
...
public:
    ...
    Solution() {}
    void reset(int n, int k, int t) {
        this->n = n;
        graph = vector<node>(n, node(0, k));
        for (int i = 0;i < n;i++) graph[i].set_nid(i);

        this->maxEdgesAllowed = k;
    ...
    int maxWeight(int n, vector<vector<int>>& edges, int k, int t) {
        ...
        if (k == 0) return 0; // path with 0 edges, return 0
        if (edges.size() == 0) return -1; // no edges
        if (n < k) return -1; // not enough nodes to make k edges
        ...
        for (auto edge : edges) {
            ...
            if (wt >= t) continue;
            graph[n1].add(1, wt, n2);
        }


        bool contineBuild = true;

        for (int curLevel = 1;curLevel < k;curLevel++) {

            if (!contineBuild) {
                LOG("*** No more levels can be built, stopping at level:" << curLevel);
                break;
            }
            contineBuild = false;
            LOG("Building level:" << curLevel + 1);
            for (int nid = 0;nid < n;nid++) {
                contineBuild |= buildLevel(nid, curLevel);
                LOG("\tnode:" << nid << " dp:" << graph[nid].dp[curLevel + 1]);
            }
        }

        for (int i = 0;i < n;i++) {
            auto& node = graph[i];
            auto& lastLevel = node.dp[maxEdgesAllowed];

            if (lastLevel.empty()) continue;

            if (resultMaxPathSum < prev(lastLevel.end())->first) {
                LOG("Node:" << i << " maxsum=" << prev(lastLevel.end())->first);
                resultMaxPathSum = prev(lastLevel.end())->first;
            }
        }
    ...
    bool buildLevel(int nid, int curLevel) {

        bool ret = false;
        // get all neighbors from previous level, find their neighbors and add to next level

        auto& node = graph[nid];
        for (auto& [curPsum, clNbrList] : node.dp[curLevel]) {
            for (auto clNbrId : clNbrList) {

                for (auto& [nextWt, nextNbrList] : graph[clNbrId].dp[1]) {
                    int newPathSum = curPsum + nextWt;
                    if (newPathSum >= maxPathSumAllowed) continue;

                    node.add(curLevel + 1, newPathSum, nextNbrList);
                    ret = true;
                }
            }
        }
