...
public:
    ...
    int catMouseGame(vector<vector<int>>& graph) {
        ...
        bfsQ.push(HOME_CAT);
        nodes[HOME_CAT].mouseD = 0;
        while (!bfsQ.empty()) {
            auto curNode = bfsQ.front();
            bfsQ.pop();

            int curDistance = nodes[curNode].mouseD;
            for (auto e : graph[curNode]) {
                if (nodes[e].mouseD == INVALID) {
                    nodes[e].mouseD = curDistance + 1;
                    bfsQ.push(e);
                }
            }
        }
        ...
        for (int i = 0;i < (n * 2);i++) {
            ...
            LOG("@move:" << i << " m=" << nodes[mousePosition] << " c=" << nodes[catPosition]);

            trace.push_back({ mousePosition, catPosition });
            if (isRepetation(trace)) {
                LOG("Draw by repetation");
                return R_DRAW;
            }

            mousePosition = makeMoveMouse(graph);
            if (mousePosition == HOLE) return R_MOUSE;
            if (mousePosition == catPosition) return R_CAT;

            catPosition = makeMoveCat(graph);
            if (mousePosition == catPosition) return R_CAT;
        }
        return R_DRAW;
    }
    ...
    int makeMoveMouse(vector<vector<int>>& graph) {

        if (graph[mousePosition][0] == HOLE) return graph[mousePosition][0];
        vector<int> options;
        set_difference(graph[mousePosition].begin(), graph[mousePosition].end(), graph[catPosition].begin(), graph[catPosition].end(), std::back_inserter(options));
        if (options.empty()) return graph[mousePosition][0];
        if (options.size() == 1) return options[0];

        size_t i = 0;
        int new_position = options[i];
        if (new_position == catPosition) new_position = options[++i];

        for (i++;i < options.size();i++) {
            if (options[i] == catPosition) continue;
            if (nodes[new_position].h_distance > nodes[options[i]].h_distance) {
                new_position = options[i];
            }
        }
        return new_position;
    }
    int makeMoveCat(vector<vector<int>>& graph) {

        int i = 0;
        int new_position = graph[catPosition][i];
        if (new_position == HOLE) new_position = graph[catPosition][++i];
        if (new_position == mousePosition) return new_position;

        for (i++;i < (int)graph[catPosition].size();i++) {
            auto option = graph[catPosition][i];
            if (option == mousePosition) return option;

            if (nodes[new_position].h_distance > nodes[option].h_distance) {
                new_position = option;
                continue;
            }
            if (nodes[new_position].mouseD > nodes[option].mouseD) {
                new_position = option;
                break;
            }
        }
...
ostream& operator<<(ostream& out, const Data& d) {
    out << "@:" << d.id << "{" << d.h_distance << "," << d.mouseD << "," << d.c_distance << "}";
    return out;
}
