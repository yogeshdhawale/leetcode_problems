...
struct Car {
    ...
    POSInt pos;
    SPDInt speed;
    Car(IDX i, POSInt pos, SPDInt speed) {
        this->idx = i;
        this->pos = pos;
        this->speed = speed;
    }
    Car(IDX i, vector<int>& car) : Car(i, car[POS_IDX], car[SPD_IDX]) {
    }
};
...
struct Data {
    ...
    void checkIntersection(IDX scIdx, vector<double>& result) {
        ...
        LOG("checkIntersection with: " << cars[scIdx]);

        while (it_speed != cMap.end()) {

            for (auto fcIdx : it_speed->second) {
                auto curResult = result[fcIdx];
                ...
                if (curResult == -1) {
                    ...
                    LOG("\t fasterCar:" << fcIdx << " result: " << curResult << " vs: " << newResult);
                    result[fcIdx] = min(curResult, newResult);
                }
