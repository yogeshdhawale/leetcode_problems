...
public:
    ...
    int maxTotalReward(vector<int>& rewardValues) {
        ...
        map<int, bool> rewardOptions;

        for (auto [k, v] : entries) {

            rewardOptions[k] = true;
            LOG("@" << k << " rewardOptions:" << rewardOptions);

            for (auto itr = rewardOptions.begin();itr != rewardOptions.end();++itr) {
                if (itr->first >= k) {
                    break;
                }
                auto newVal = itr->first + k;
                rewardOptions[newVal] = true;
            }
        }
        LOG("@End: rewardOptions:" << rewardOptions);
        return rewardOptions.rbegin()->first;
    }
