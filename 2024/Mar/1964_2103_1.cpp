...
struct Data {
    ...
    int AddChild(int val) {
        ...
        for (auto itr = children.begin(); itr != children.end(); itr++) {
            ...
            res = data.AddChild(val);
            data.end = max(data.end, val);
...
public:
    ...
    vector<int> longestObstacleCourseAtEachPosition(vector<int>& obstacles) {
        ...
        vector <pair<int, Data*>> root;

        for (size_t i = 0; i < size; i++) {
            auto target = obstacles[i];
            int maxdepth = 0;
            bool processed = false;
            ...
            for (auto itr = root.begin(); itr != root.end(); itr++) {
                ...
                Data* obj = itr->second;
                Data& data = *obj;
                int res = data.AddChild(target);
                LOG("\tCalled AddChild on " << data);
                maxdepth = max(maxdepth, res);
            }
            if (!processed) {
                root.emplace_back(target, new Data(target));
            }
            result[i] = 1 + maxdepth;
            LOG("i:" << i << " target: " << target << " maxdepth:" << maxdepth + 1);
            LOG(root);
...
ostream& operator<<(ostream& out, const Data& d) {
    ...
    out << "{\"D[" << d.start << "-" << d.end << "]_" << d.children.size() << "\"";
...
void test1(int testID)
{
...
#endif
    {
        ...
        cout << TEXT_LOG_PREFIX << "test loop: " << i << " ==> " << result << " vs " << expected[i] << endl;
