...
struct RangeHash {
    ...
    {
        return hash<int>()(range.first) ^ hash<int>()(range.second);
    }
};
...
struct Data {
    ...
    map<POSITION, bool> unmatchedPositions;
    vector<VALUE> values;
    ...
    unordered_map<RANGE, bool, RangeHash> cache;
    ...
    void AddGroup() {
        ...
        for (size_t i = 0; i < 26; i++) {
            ...
            if (cache.find({ vp.begin()->first, vp.rbegin()->first }) != cache.end()) {
                continue;
            }
        ...
        if (curRange.first == range.first) {
            ReduceCount(values[curRange.first], curRange.first);
    ...
    int ExtendWindow(VALUE v, POSITION p) {
        ...
        if (prev.second != INVALID) {
            if (abs(v - prev.second) > 2) {
                result = EndWindow(range.first, p - 1);
            }
        }
    ...
    void ReduceCount(VALUE v, POSITION pos) {

        if (counts[v].size() == 0) return;
        ...
        if (counts[v].size() < k) {
            ...
            counts[v].erase(counts[v].begin());
            for (auto pos : counts[v])
                unmatchedPositions[pos.first] = true;
        }
    }

    void shrinkWindow(POSITION pos) {
        for (auto i = range.first;i <= pos;i++) {
            ReduceCount(values[i], i);
            AddGroup();
        }
        range.first = pos + 1;
    }
