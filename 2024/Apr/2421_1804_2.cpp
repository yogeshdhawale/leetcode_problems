...
struct Node {
    NODE_ID id;
    WEIGHT weight;
    map<WEIGHT, vector<Node*>> neighbors;
    ...
    void AddNeighbor(Node& other) {
        neighbors[other.weight].push_back(&other);
    }

    void traverse(WEIGHT startw, vector<bool>& visited, int& curSize, vector<bool>& pathMap) {
        ...
        for (auto [w, list] : neighbors) {
            ...
            for (auto& ptr : list) {
                ...
                nextNode.traverse(startw, visited, curSize, pathMap);
            }
        }
        return;
    }
...
struct Data {
    ...
    int getValidPaths() {
        int result = 0;
        int curSize = 0;
        vector<bool> pathMap(totalNodes, false);
        vector<bool> visited(totalNodes, false);

        for (auto& w : wmap) {

            if (w.second.size() <= 1) continue;
            for (auto& nid : w.second) {
                if (pathMap[nid]) continue;
                ...
                node.traverse(node.weight, visited, curSize, pathMap);
                auto res = (curSize * (curSize - 1)) / 2;
                result += res;

                LOG("node:" << node.id << " curSize:" << curSize << " res:" << res << " result:" << result);
            }
        }
        return result;
    }
...
ostream& operator<<(ostream& out, const Node& d) {
    ...
    out << "{id:" << d.id << " weight:" << d.weight << " : ";

    for (auto& [w, list] : d.neighbors) {
        out << " {w:" << w << " size(" << list.size() << ") : ";
        for (auto& ptr : list) out << ptr->id << " ";
    }
    out << "} ";
