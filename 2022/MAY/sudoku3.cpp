...
public:
    ...
    void reSetValue(int i, int j, int v)
    {
        CellProperty pr = prop[i][j];
        pr.value = -1;
        pr.isFound = false;
        ...
        while (foundValues < totalValues)
        {
            ...
            for (int i = 0; i < r; i++)
            {
                for (int j = 0; j < c; j++)
                {
                    ...
                    int flag = rFlags[pr.rid] | cFlags[pr.cid] | gFlags[pr.gid];

                    if (elements.find(flag) != elements.end())
                    {
                        int v = elements[flag];
                        setValue(i, j, v);
                        pr.isFound = true;
    ...
    void findGuess(int &gi, int &gj, int &gv)
    {
        for (int i = 0; i < r; i++)
        {
            for (int j = 0; j < c; j++)
            {
                ...
                {
                    int flag = rFlags[i] | cFlags[j] | gFlags[prop[i][j].get().gid];
