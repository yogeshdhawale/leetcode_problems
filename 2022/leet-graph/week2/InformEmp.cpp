...
public:
    ...
    void buildTree()
    {
        ...
        {
            mgr2sub[list[i]].push_back(i);
        }

        allTime = processSubtree(rootID);
    }

    int processSubtree(int id)
    {
        if (mgr2sub.find(id) == mgr2sub.end())
        {
            return informTime[id];
        }

        int maxTime = 0;
        for (int i = 0; i < mgr2sub[id].size(); i++)
        {
            int curEmp = mgr2sub[id][i];
            int time = processSubtree(curEmp);
            if (maxTime < time)
                maxTime = time;
        }
