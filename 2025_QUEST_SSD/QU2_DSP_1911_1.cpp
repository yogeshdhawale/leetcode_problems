...
struct Suffix {
    ...
    bool checkChar(char c) {
        ...
        auto itr = trackers.begin();
        while (itr != trackers.end()) {
            auto& pos = *itr;
            if (s[pos] == c) {
                pos++;
                itr++;
        ...
        if (c == firstchar) trackers.push_front(1);
        if (trackers.empty()) return false;

        //LOG("for " << c << " trackers:" << trackers << " for s:" << s);

        if (*trackers.rbegin() == len) {
            trackers.pop_back();
            return true;
        }
        return false;
...
public:
    ...
    StreamChecker(vector<string>& words) {
        ...
        for (auto word : words)
            suffixes.push_back(Suffix(word));
    }
