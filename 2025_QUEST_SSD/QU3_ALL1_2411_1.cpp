...
struct TestCaseStruct
{
    ...
    void setInput(V1Str input1, V2Str input2) {
        this->mData1 = input1;
        this->mData2 = input2;
    }
...
struct StringInfo {
    ...
    StringInfo(int _id, string _str) : id(_id), str(_str), count(1) {}
...
public:
    ...
    map<int, unordered_set<int>> count2ids;
    vector<StringInfo> infos;
    ...
    void inc(string key) {
        if (s2id.find(key) == s2id.end()) {
            infos.push_back(StringInfo(infos.size(), key));
            auto sid = infos.back().id;
            s2id[key] = sid;
            count2ids[1].insert(sid);
            return;
        }

        auto sid = s2id[key];
        auto& sinfo = infos[sid];
        int oldCount = sinfo.count;
        sinfo.count++;

        count2ids[oldCount].erase(sid);
        if (count2ids[oldCount].empty()) {
            count2ids.erase(oldCount);
        }
        count2ids[oldCount + 1].insert(sid);

        LOG("\ttotal count:" << count2ids.size());
    }
    ...
    void dec(string key) {
        ...
        //if (sinfo.count == 0) s2id.erase(key);
        if (sinfo.count != 0) count2ids[oldCount - 1].insert(sid);

        LOG("\ttotal count:" << count2ids.size());
    }

    string getMaxKey() {
        if (count2ids.empty()) return "";

        auto it = count2ids.rbegin();
        int maxCount = it->first;
        auto& idset = it->second;
        if (idset.empty()) return "";

        LOG("maxCount:" << maxCount << " ids:" << idset);
        auto sid = *(idset.begin());
        return infos[sid].str;
    }

    string getMinKey() {
        if (count2ids.empty()) return "";

        auto it = count2ids.begin();
        int minCount = it->first;
        auto& idset = it->second;
        if (idset.empty()) return "";

        LOG("minCount:" << minCount << " ids:" << idset);
        auto sid = *(idset.begin());
        return infos[sid].str;
    }
};
    ...
    int proxyFunction(TestCaseStruct& tc) {
        ...
        for (size_t i = 0; i < tc.mData1.size(); i++) {
            ...
            if (cmd == "AllOne") {
                ...
                obj->inc(tc.mData2[i][0]);
            }
            else if (cmd == "dec") {
                obj->dec(tc.mData2[i][0]);
            }
            else if (cmd == "getMaxKey") {
                string ret = obj->getMaxKey();
                LOG("getMaxKey:" << ret);
            }
            else if (cmd == "getMinKey") {
                string ret = obj->getMinKey();
                LOG("getMinKey:" << ret);
            }
            else {
