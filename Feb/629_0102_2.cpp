...
public:
    ...
    void SetPairCnt(size_t i, long long count)
    {
        if (i > k)
        {
            return;
        }
        pairCounts[i] = (count % RESULT_MOD);
    }
...
public:
    int kInversePairs(int n, int k)
    {
        ...
        for (int i = 3; i <= size; i++)
        {
            ...
            // calculateNextLevel(i, prevcache, cache);

            int maxIdx = i * (i - 1) / 2;
            cache->SetPairCnt(0, 1);
            cache->SetPairCnt(maxIdx, 1);

            int x = 0;
            for (x = 1; x <= k; x++)
            {
                if (prevcache->GetCnt(x - 1) == 0)
                {
                    break;
                }

                long long incrVal = (cache->GetCnt(x - 1) + prevcache->GetCnt(x));
                if (x < i)
                {
                    cache->SetPairCnt(x, incrVal);
                    cache->SetPairCnt(maxIdx - x, incrVal);
                }
                else
                {
                    incrVal -= (prevcache->GetCnt(x - i));
                    if (incrVal < 0)
                    {
                        incrVal += RESULT_MOD;
                    }
    ...
    void calculateNextLevel(int i, Data *prevcache, Data *cache)
    {
        for (int j = 0; j < i / 2; j++)
        {
