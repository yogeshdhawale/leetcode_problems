...
private:
    ...
    map<string, vector<string>> cuisinesMap;
...
public:
    FoodRatings(vector<string> &foods, vector<string> &cuisines, vector<int> &ratings)
    {
        ...
        for (size_t i = 0; i < foods.size(); i++)
        {
            ...
            cuisinesMap[cuisines[i]].push_back(foods[i]);
        }
    }
    ...
    string highestRated(string cuisine)
    {
        int rating = -1;
        string result = "";
        vector <string> & cuisinesInfo = cuisinesMap[cuisine];
        for (auto x : cuisinesInfo)
        {

            if (ratingMap[x] > rating)
            {
                rating = ratingMap[x];
                result = x;
            }
            else if (ratingMap[x] == rating && x < result)
            {
                result = x;
            }
        }
